...
use crate::ast::{AddressingMode, Ast, InstrDir, Opcode};
...
pub trait Assemblable {
    fn size(&self) -> usize;
    fn assemble(&self) -> Result<Vec<u8>, AssemblerError>;
}
...
impl Assembler {
    ...
    pub fn assemble(&mut self, ast: &Ast) -> Result<Vec<u8>, AssemblerError> {
        ...
        while !self.unresolved_refs.is_empty() && pass_count < 5 {
            if self.verbose {
                println!(
                    "Pass {}: {} unresolved references remain",
                    pass_count,
    ...
    fn resolve_labels(&mut self, ast: &Ast) -> Result<(), AssemblerError> {
        ...
        for line in ast.lines() {
            if let Some(instrdir) = &line.instrdir() {
                // Process instructions and directives
                match instrdir {
                    InstrDir::Directive(name, args) => {
                        // Handle directives separately
                        match name.as_str() {
                            "org" => {
                                let value = self.parse_value(&args.join(""))?;
                                self.origin = value;
                                self.pc = value;
                            }
            ...
            if let Some(label) = &line.label() {
                ...
                if self.labels.contains_key(&label.name) {
                    return Err(self.line_error(format!("Duplicate label: {}", label.name)));
                }

                // Store the label address
                self.labels.insert(label.name.clone(), self.pc);
            }
            if let Some(instrdir) = &line.instrdir() {
                match instrdir {
                    InstrDir::Instruction(_, operand) => {
                        // Handle instructions
                        if let Some(operand) = operand {
                            /*
                            // Check for unresolved references in operands
                            if let Some(label) = operand.get_label() {
                                self.unresolved_refs.push((self.pc, label.clone(), false));
                            }
                            */
                        }
                    }
    ...
    fn generate_code(&mut self, ast: &Ast) -> Result<(), AssemblerError> {
        ...
        for line in ast.lines() {
            match line.instrdir() {
                Some(InstrDir::Instruction(_, _)) => {}
                _ => {}
            }
            if let Some(instrdir) = &line.instrdir() {
                // Calculate the size of the instruction
                let size = self.instruction_size(instrdir)?;
    ...
    fn resolve_references(&mut self) -> Result<(), AssemblerError> {
        ...
        for (pos, label, is_relative) in &self.unresolved_refs {
            if let Some(&addr) = self.labels.get(label) {
                ...
                if *pos >= self.binary.len() {
                    if self.verbose {
                        println!(
                            "Skipping reference to '{}' at ${:04X} (beyond binary size)",
                            label, pos
                        );
                    }
                    continue;
                ...
                if *is_relative {
                    ...
                    if (addr as isize - current_pos as isize) > 127
                        || (addr as isize - current_pos as isize) < -128
                    {
                        return Err(AssemblerError::ValueOutOfRange(format!(
                            "Branch to '{}' is too far (offset: {})",
                            label,
                            addr as isize - current_pos as isize
                        )));
                    }
                ...
                if self.verbose {
                    println!(
                        "Resolved reference to '{}' at ${:04X} -> ${:04X}",
                        label, *pos, addr
                    );
                }
            } else {
        ...
        if !self.unresolved_refs.is_empty() {
            ...
            return Err(AssemblerError::UnknownLabel(format!(
                "Unresolved labels after multiple passes: {:?}",
                missing_labels
            )));
        }
    ...
    fn instruction_size(&self, instruction: &InstrDir) -> Result<usize, AssemblerError> {
        match instruction {
            InstrDir::Instruction(opcode, operand) => {
                if let Some(operand) = &operand {
                    let addr_mode = operand.get_addressing_mode(opcode);
            ...
            InstrDir::Directive(name, value) => {
                // Handle directives separately
                match name.as_str() {
                    "org" => Ok(0), // No size for .org
                    // FIXME  handle other directives
                    /*
                    "byte" | "db" => {
                        // Size is the number of bytes in the directive value
                        // FIXME
                        let values: Vec<&str> =
                            value.split(",").map(|v| v.trim()).collect();
                        Ok(values.len())
                    }
                    "word" | "dw" => Ok(2 * value.split(',').count()), // 2 bytes per word
                    */
                    _ => Ok(0), // Other directives have no size
                }
            }
        }
    }
    ...
    fn encode_instruction(&mut self, instrdir: &InstrDir) -> Result<Vec<u8>, AssemblerError> {
        let mut bytes = vec![];
        match instrdir {
            InstrDir::Instruction(opcode, operand) => {
                // Determine the addressing mode
                let addr_mode = if let Some(operand) = &operand {
                    operand.get_addressing_mode(opcode)
                } else {
                    AddressingMode::Implied
                };

                let opcode_byte = self.get_opcode_byte(opcode, addr_mode)?;
                ...
                if let Some(operand) = &operand {
                    match addr_mode {
                        ...
                        AddressingMode::Immediate => {
                            ...
                            if value > 0xFF {
                                return Err(self.line_error(format!(
                                    "Immediate value out of range: {} > 0xFF",
                                    value
                                )));
                            }
                        ...
                        | AddressingMode::ZeroPageY => {
                            ...
                            if value > 0xFF {
                                return Err(self.line_error(format!(
                                    "Zero page address out of range: {} > 0xFF",
                                    value
                                )));
                            }
                            bytes.push((value & 0xFF) as u8);
                        ...
                        | AddressingMode::AbsoluteY => {
                            ...
                            if value > 0xFFFF {
                                return Err(self.line_error(format!(
                                    "Absolute address out of range: {} > 0xFFFF",
                                    value
                                )));
                            }
                            bytes.push((value & 0xFF) as u8);
                        ...
                        AddressingMode::Indirect => {
                            let operand_str = operand.to_string();
                            let value_str =
                                operand_str.trim_start_matches('(').trim_end_matches(')');
                            let value = self.parse_value(value_str)?;
                            if value > 0xFFFF {
                                return Err(self.line_error(format!(
                                    "Indirect address out of range: {} > 0xFFFF",
                                    value
                                )));
                            }
                            bytes.push((value & 0xFF) as u8);
            ...
            InstrDir::Directive(_, _) => {
                // Directives are handled separately
            }
        }
    ...
    fn get_opcode_byte(
        ...
        static OPCODE_TABLE: once_cell::sync::Lazy<
            HashMap<(Opcode, AddressingMode), self::opcodes::OpcodeEntry>,
        ...
        if let Some(entry) = OPCODE_TABLE.get(&(*opcode, addr_mode)) {
            ...
            Err(AssemblerError::InvalidAddressingMode(format!(
                "Invalid addressing mode {:?} for opcode {:?}",
                addr_mode, opcode
            )))
        }
    }
    ...
    fn parse_value(&mut self, value: &str) -> Result<usize, AssemblerError> {
        ...
        if value.starts_with('$') {
            ...
            // Binary
            let bin_str = &value[1..];
            return usize::from_str_radix(bin_str, 2)
                .map_err(|_| AssemblerError::Parse(format!("Invalid binary value: {}", value)));
        } else if value.chars().all(|c| c.is_digit(10)) {
            // Decimal
            return value
                .parse::<usize>()
                .map_err(|_| AssemblerError::Parse(format!("Invalid decimal value: {}", value)));
        }
        ...
        if let Some(ref ast) = self.ast {
            constant_value = ast.constant(value);
        }
        ...
        self.unresolved_refs
            .push((self.pc, value.to_string(), false));
        Ok(0) // Placeholder
    }
    ...
    fn evaluate_expression(&self, expr: &str) -> Result<usize, AssemblerError> {
        ...
        Err(AssemblerError::InvalidExpression(format!(
            "Invalid expression: {}",
            expr
        )))
    }
                ...
                match name.as_str() {
                    ...
                    "word" | "dw" => {
                        ...
                        for value_str in values {
                            ...
                            if value > 0xFFFF {
                                return Err(self.line_error(format!(
                                    "Word value out of range: {} > 0xFFFF",
                                    value
                                )));
                            }
                            self.binary.push((value & 0xFF) as u8);
